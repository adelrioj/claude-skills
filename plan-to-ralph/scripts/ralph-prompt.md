...
1. **`tasks/prd.json`** — The product requirements. Contains user stories with acceptance criteria. The `description` field references the implementation plan path.
2. **`tasks/progress.txt`** — Iteration log from previous iterations.
3. **`tasks/findings.md`** — Cross-iteration knowledge: discoveries, errors, and decisions from previous iterations.
4. **Implementation plan** — The path is in the `prd.json` `description` field (e.g., "See docs/plans/..."). Read this plan to understand file paths, code structure, and TDD steps.
...
4. **What decisions were made?** — Check `findings.md` for architecture choices that constrain this story.
