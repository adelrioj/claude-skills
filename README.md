...
| `/ship-it` | Pure conductor: spec → reviewed PR in one invocation. Chains `/spec-review-codex`, plan-writing, `/plan-to-dex`, PR creation, and `/review-pr` fix passes. Autonomous, best-effort; records leftover findings instead of halting. | `codex`, `dex`, `pr-review-toolkit`, `commit-commands` |
