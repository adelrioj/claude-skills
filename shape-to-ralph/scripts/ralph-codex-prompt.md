# Ralph Iteration Prompt (Shape-Driven — Codex)
...
1. **`AGENTS.md`** — Project conventions, build commands, architecture overview, and coding standards. Read this before writing any code.
2. **`tasks/prd.json`** — User stories with acceptance criteria. The `description` field references the shaping doc.
3. **`tasks/progress.txt`** — Iteration log from previous iterations.
4. **`tasks/findings.md`** — Cross-iteration knowledge: architecture decisions, requirements, errors, and discoveries.
5. **Shaping doc** — The path is in the `prd.json` `description` field (e.g., "See docs/shaping/..."). Contains requirements, shape parts, affordance tables, and wiring diagrams. Read this to understand the _why_ and the _system design_.
6. **Slices doc** — Referenced in story `notes` as "Source: Slice VN from [path]". Contains per-slice affordance tables and file tables.
...
Also read any module-level documentation files (e.g., `AGENTS.md` files in subdirectories) if they exist.
...
- **Follow project conventions**: Read and follow all conventions defined in `AGENTS.md`
...
6. **Follow AGENTS.md conventions** — read project-specific rules before coding.
