...
### `/ship-it`
Pure conductor that chains the existing units into one autonomous spec→PR pipeline. Six-step sequential process: hardens the spec via `/spec-review-codex`, writes the implementation plan, executes it via `/plan-to-dex`, opens a PR with `/commit-commands:commit-push-pr`, runs `/review-pr` and fixes CRITICAL/IMPORTANT findings (up to 3 passes); merging stays manual. A preflight (codex + dex on PATH, `pr-review-toolkit` + `commit-commands` plugins present via an on-disk command-file check, spec located) is the sole hard abort. Two **execution modes** control context: in **monitored** mode (default) the heavy Skill steps run in the main loop with trailing boundary-verifier subagents (after spec-review and execution) for live observability; in **isolated** mode Steps 1/2/4 run as execute-and-report subagents that return only a structured contract, keeping raw codex/dex output out of the conductor's context. Steps 3/5/6 always run in the main loop; the PR-review verifier runs in both modes. Never halts on quality findings; records any leftover issues in a final report. Requires the `codex` and `dex` CLIs plus the `pr-review-toolkit` and `commit-commands` plugins.
...
- `/ship-it` has two execution modes — **monitored** (default; heavy Skill steps run in the main loop with trailing boundary verifiers, for observability) and **isolated** (Steps 1/2/4 run as execute-and-report subagents returning only a three-field contract, keeping raw codex/dex output out of the conductor's context). Steps 3/5/6 always run in the main loop; the zero-diff check is always verified against git in the conductor's own shell, never from a subagent summary, so a mis-summary can't fabricate a PR
