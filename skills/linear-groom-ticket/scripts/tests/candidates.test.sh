...
# search result (NBS-100, NBS-238-self, a dup of NBS-101) and one open-state
# list (NBS-101, NBS-100 again — the dup must collapse).
stage_parts() {
  local rd="$1"
...
{"result":{"issues":[
  {"identifier":"NBS-238","title":"self must be excluded","state":{"name":"To Do"},"url":"u0"},
  {"identifier":"NBS-100","title":"join predicate dropped","state":{"name":"To Do"},"url":"u1"},
  {"identifier":"NBS-101","title":"unrelated","state":{"name":"Backlog"},"url":"u2"}
]}}
JSON
  cat >"$rd/.candidate-parts/open-1.json" <<'JSON'
{"result":{"issues":[
  {"identifier":"NBS-100","title":"join predicate dropped","state":{"name":"To Do"},"url":"u1"},
  {"identifier":"NBS-102","title":"only in open list","state":{"name":"In Progress"},"url":"u3"}
]}}
JSON
}
...
stage_parts "$RD"
# NBS-100 carries the ticket's own spec URL — a shared identifier.
printf '{"result":{"issue":{"description":"see https://example.invalid/spec for the JOIN work"}}}\n' \
  >"$RD/.candidate-parts/body-NBS-100.json"
printf '{"result":{"issue":{"description":"nothing in common here"}}}\n' \
  >"$RD/.candidate-parts/body-NBS-101.json"
printf '{"result":{"issue":{"description":"open-only candidate body"}}}\n' \
  >"$RD/.candidate-parts/body-NBS-102.json"

python3 "$CAND" --run-dir "$RD" --ticket NBS-238 >"$TMP/c.out" 2>"$TMP/c.err" \
...
assert "NBS-238" not in idents, "ticket must exclude itself: %r" % idents
assert idents.count("NBS-100") == 1, "NBS-100 must appear once (dup collapsed): %r" % idents
assert set(idents) == {"NBS-100", "NBS-101", "NBS-102"}, idents
...
assert "https://example.invalid/spec" in (byid["NBS-100"].get("identifiers") or []), byid["NBS-100"]
assert byid["NBS-100"]["description_chars"] and byid["NBS-100"]["excerpt"], byid["NBS-100"]
assert d["bodies_fetched"] == 3, d
PY

# --- the body-read cap marks later candidates skipped, never fetched -------
RD2="$TMP/state2/linear-groom/NBS-238"
stage_parts "$RD2"
for id in NBS-100 NBS-101 NBS-102; do
  printf '{"result":{"issue":{"description":"body of %s"}}}\n' "$id" >"$RD2/.candidate-parts/body-$id.json"
done
LINEAR_GROOM_BODY_LIMIT=1 python3 "$CAND" --run-dir "$RD2" --ticket NBS-238 >"$TMP/c2.out" 2>"$TMP/c2.err" \
...
RD3="$TMP/state3/linear-groom/NBS-238"
stage_parts "$RD3"
printf '{"result":{"issue":{"description":"fine"}}}\n' >"$RD3/.candidate-parts/body-NBS-100.json"
printf 'get_issue NBS-101 failed — description unavailable\n' >"$RD3/.candidate-parts/bodyerr-NBS-101.txt"
printf '{"result":{"issue":{"description":"fine too"}}}\n' >"$RD3/.candidate-parts/body-NBS-102.json"
python3 "$CAND" --run-dir "$RD3" --ticket NBS-238 >"$TMP/c3.out" 2>"$TMP/c3.err" \
...
assert byid["NBS-101"]["excerpt"] is None and "excerpt_error" in byid["NBS-101"], byid["NBS-101"]
assert d["bodies_fetched"] == 2, d
PY

# --- missing parts dir is a hard, explicit failure ------------------------
RD4="$TMP/state4/linear-groom/NBS-238"
mkdir -p "$RD4"
cp "$HERE/fixtures/ticket-complete.json" "$RD4/ticket.json"
if python3 "$CAND" --run-dir "$RD4" --ticket NBS-238 >"$TMP/c4.out" 2>"$TMP/c4.err"; then
